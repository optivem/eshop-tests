...
2. **Test approval** — after the Test Agent writes the acceptance tests (AT · RED · WRITE TEST), the human reviews the test code before it is committed. This catches translation errors from Gherkin to code before DSL, drivers, and backend are built on top.
3. **DSL approval** — after the DSL Agent implements the DSL (AT · RED · WRITE DSL), the human reviews the DSL design and driver interface signatures before they are committed. DSL method names and driver DTOs are the architectural contract — errors here cascade into all downstream agents.
4. **Driver approval** — after the Driver Agent completes AT · RED · WRITE DRIVER, the human reviews the driver implementation before backend/frontend work begins. This validates the full test spec (tests + DSL + drivers) as a unit, preventing agents from chasing false failures caused by a wrong driver.
...
[Story Agent]      →  Gherkin scenarios
    ...
    │                                                               │
    │  [Test Agent]    →  Acceptance tests   AT · RED · WRITE TEST  │
    │      │                                                        │
    │      │                              ← HUMAN APPROVES TESTS    │
    │      │                                                        │
    │  [Test Agent]    →  Commit tests     AT · RED · COMMIT TEST   │
    │      │                                                        │
    │      ▼                                                        │
    │  [DSL Agent]     →  DSL + interfaces  AT · RED · WRITE DSL    │
    │      │                                                        │
    │      │                              ← HUMAN APPROVES DSL      │
    │      │                                                        │
    │  [DSL Agent]     →  Commit DSL        AT · RED · COMMIT DSL   │
    │      │                                                        │
    │      ▼                                                        │
    │  [Driver Agent]  →  Drivers          AT · RED · WRITE DRIVER  │
    │      │                                                        │
    │      │                              ← HUMAN APPROVES DRIVERS  │
    │      │                                                        │
    │  [Driver Agent]  →  Commit drivers   AT · RED · COMMIT DRIVER │
    │      │                                                        │
    │      ├── external/ changed? ► contract-tests sub-process      │
    │      │                                                        │
    │  [Backend Agent] →  Working backend  AT · GREEN · WRITE SYSTEM│
    │      │                                                        │
    │      ▼                                                        │
    │  [Frontend Agent]→  Working frontend AT · GREEN · WRITE SYSTEM│
    │      │                                                        │
    │      ▼                                                        │
    │  [Release Agent] →  Final commit    AT · GREEN · COMMIT SYSTEM│
    │      │                                                        │
...
- **New DSL needed (compile errors in AT · RED · WRITE TEST):** The Test Agent implements only the **first scenario** and leaves the rest as `// TODO:` comments. Each loop cycle picks up the next scenario. This validates the DSL design before multiplying it across all scenarios.
...
- **COMMIT output:** Committed acceptance tests (`@Disabled("AT · RED · WRITE TEST")`)
- **Governed by:** `acceptance-tests.md` — AT · RED · WRITE TEST and AT · RED · COMMIT TEST phases
...
- **COMMIT output:** Driver stubs added, tests committed (`@Disabled("AT · RED · WRITE DSL")`)
- **Governed by:** `acceptance-tests.md` — AT · RED · WRITE DSL and AT · RED · COMMIT DSL phases; `dsl-core.md` for coding rules
...
- **COMMIT output:** Tests committed (`@Disabled("AT · RED · WRITE DRIVER")`)
- **Governed by:** `acceptance-tests.md` — AT · RED · WRITE DRIVER and AT · RED · COMMIT DRIVER phases; `driver-port.md` for coding rules
- **Handoff:** Orchestrator routes to contract-tests sub-process or AT · GREEN · WRITE SYSTEM based on DSL agent's external system flag

### Backend Agent
- **Input:** Driver interfaces, existing backend codebase
- **Output:** Backend changes that make API acceptance tests pass
- **Governed by:** `acceptance-tests.md` — AT · GREEN · WRITE SYSTEM (backend)
...
- **Governed by:** `acceptance-tests.md` — AT · GREEN · WRITE SYSTEM (frontend)
...
- **Output:** `@Disabled` removed, final commit `<Scenario> | AT · GREEN · COMMIT SYSTEM`
- **Governed by:** `acceptance-tests.md` — AT · GREEN · COMMIT SYSTEM phase
