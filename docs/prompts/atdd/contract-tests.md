...
_This process is only triggered when the DSL Agent (AT · RED · WRITE DSL) reports **external system interfaces changed = yes** — i.e. new methods were added to interfaces under `external/` (e.g. `driver-port/.../external/erp`). It is initiated by the orchestrator after AT · RED · COMMIT DRIVER._

_If the External System does not even exist yet, make Smoke Tests pass first._

## CT · RED · WRITE TEST (STOP)
...
3. Verify that they fail when executed against the Stub External System:
   ...
   ```
4. Mark the tests as disabled with reason `"CT · RED · WRITE TEST"` (see `language-equivalents.md` for syntax).
5. STOP. Present the contract tests to the user and ask for approval. Do NOT continue.

## CT · RED · COMMIT TEST

1. If there were compile-time errors in CT · RED · WRITE TEST:
...
2. COMMIT with message `<Scenario> | CT · RED · COMMIT TEST`.
3. STOP. Do not proceed further. Phase progression is controlled by the orchestrator, not by this agent.

## CT · RED · WRITE DSL (STOP)

1. Enable the tests marked disabled with reason `"CT · RED · WRITE TEST"`.
...
2. Run the tests and verify they fail with a runtime error:
   ...
   .\Run-SystemTests.ps1 -Suite <suite-contract-stub> -Test <TestMethodName>
   ```
3. Mark the tests as disabled with reason `"CT · RED · WRITE DSL"` (see `language-equivalents.md` for syntax).
4. COMMIT with message `<Scenario> | CT · RED · COMMIT DSL`.
5. Automatically proceed to CT · RED · WRITE DRIVER (STOP).

## CT · RED · WRITE DRIVER (STOP)

1. Enable the tests marked disabled with reason `"CT · RED · WRITE DSL"`.
...
## CT · RED · COMMIT DRIVER

1. Mark the tests as disabled with reason `"CT · RED · WRITE DRIVER"` (see `language-equivalents.md` for syntax).
2. COMMIT with message `<Scenario> | CT · RED · COMMIT DRIVER`.
3. STOP. Do not proceed further. Phase progression is controlled by the orchestrator, not by this agent.

## CT · GREEN · WRITE STUBS (STOP)

1. Enable the tests marked disabled with reason `"CT · RED · WRITE DRIVER"`.
...
## CT · GREEN · COMMIT STUBS

1. Remove the disabled annotation (reason `"CT · RED · WRITE DRIVER"`) from the tests.
2. Run the tests and verify they pass.
3. COMMIT with message `<Scenario> | CT · GREEN · COMMIT STUBS`.
