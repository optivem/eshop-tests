...
- `Ext*Request` DTOs must use only `String` fields — never `Boolean`, `Integer`, `Double`, or other non-String types. This allows invalid values to pass through for negative test scenarios. Type conversion happens inside the HTTP client or serialization layer.
- `Ext*Response` DTOs may use typed fields (e.g. `BigDecimal`, `Boolean`) since they are only used for deserialization, not for constructing negative test inputs.
- All external DTOs use Lombok: `@Data @Builder @NoArgsConstructor @AllArgsConstructor`.
...
The shop API driver uses a controller-per-resource pattern. `ShopApiClient` composes multiple controllers (e.g. `OrderController`, `CouponController`, `ReviewController`), each managing one API endpoint group.
